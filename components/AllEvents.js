...
export default function AllEvents() {
    ...
    return <div>
        <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-5 p-4'>
            {
                data.map(e => <Link title={`visit to view events on ${e.season}`} className='shadow group rounded hover:shadow-md  hover:-translate-y-1 hover:bg-green-100 duration-200 transition-all overflow-hidden bg-white' href={`/seasons/${e.season.toString().split(' ').join('-') }`} key={e.season}>
                    <figure>
