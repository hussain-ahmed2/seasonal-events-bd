...
export default function Events() {
    ...
    return <div>
        ...
        <div className='flex flex-col gap-5 p-4'>
            {
                data.map(e => <Link className='flex gap-5 flex-col md:flex-row md:even:flex-row-reverse hover:bg-green-100 rounded overflow-hidden group shadow-sm' href={`/events/${e.season.toString().split(' ').join('-')}`} key={e.season}>
